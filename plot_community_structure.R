...
the_row = high_ranks[i,]
the_row_no_underscore = the_row[the_row != "_"]
the_diff = length(the_row) - length(the_row_no_underscore)
if(the_diff > 0){
last_term = tail(the_row_no_underscore, n=1)
complement_row = rep(last_term, the_diff)
the_new_row = c(the_row_no_underscore, complement_row)
list_of_vecs[[length(list_of_vecs) + 1]] <- the_new_row
}else{
  list_of_vecs[[length(list_of_vecs) + 1]] <- the_row
}
}

high_ranks = as.data.frame(do.call(rbind, list_of_vecs))
###################################################################
###################################################################
###################################################################
